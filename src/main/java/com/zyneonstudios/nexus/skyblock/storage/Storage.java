package com.zyneonstudios.nexus.skyblock.storage;

import com.zyneonstudios.nexus.skyblock.storage.types.Config;
import com.zyneonstudios.nexus.skyblock.storage.types.MySQL;
import com.zyneonstudios.nexus.skyblock.storage.types.SQLite;
...
public class Storage {
    ...
    public Object get(String table, String path, int row) {
        if (type == storageType.YAML) {
            if(yaml.getCFG().contains(table + "." + path + ".content" + row)) {
    ...
    public void ensure(String path, Object content) {
        if(get(path) == null) {
            set(path, content);
        }
    }
